/*
Vayu API

The Vayu API is a RESTful API that allows you to submit events for processing and storage & manage billing related entities.           The API is secured using the Bearer Authentication scheme with JWT tokens.           To obtain a JWT token, please contact Vayu at team@withvayu.com

API version: 1.0.0
Contact: dev@withvayu.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the NetSuiteExportSalesOrderRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &NetSuiteExportSalesOrderRequest{}

// NetSuiteExportSalesOrderRequest struct for NetSuiteExportSalesOrderRequest
type NetSuiteExportSalesOrderRequest struct {
	ContractId string `json:"contractId" validate:"regexp=^[0-9a-fA-F]{24}$"`
	ProductsIds []string `json:"productsIds"`
	SubsidiaryId string `json:"subsidiaryId"`
}

type _NetSuiteExportSalesOrderRequest NetSuiteExportSalesOrderRequest

// NewNetSuiteExportSalesOrderRequest instantiates a new NetSuiteExportSalesOrderRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNetSuiteExportSalesOrderRequest(contractId string, productsIds []string, subsidiaryId string) *NetSuiteExportSalesOrderRequest {
	this := NetSuiteExportSalesOrderRequest{}
	this.ContractId = contractId
	this.ProductsIds = productsIds
	this.SubsidiaryId = subsidiaryId
	return &this
}

// NewNetSuiteExportSalesOrderRequestWithDefaults instantiates a new NetSuiteExportSalesOrderRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNetSuiteExportSalesOrderRequestWithDefaults() *NetSuiteExportSalesOrderRequest {
	this := NetSuiteExportSalesOrderRequest{}
	return &this
}

// GetContractId returns the ContractId field value
func (o *NetSuiteExportSalesOrderRequest) GetContractId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ContractId
}

// GetContractIdOk returns a tuple with the ContractId field value
// and a boolean to check if the value has been set.
func (o *NetSuiteExportSalesOrderRequest) GetContractIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ContractId, true
}

// SetContractId sets field value
func (o *NetSuiteExportSalesOrderRequest) SetContractId(v string) {
	o.ContractId = v
}

// GetProductsIds returns the ProductsIds field value
func (o *NetSuiteExportSalesOrderRequest) GetProductsIds() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.ProductsIds
}

// GetProductsIdsOk returns a tuple with the ProductsIds field value
// and a boolean to check if the value has been set.
func (o *NetSuiteExportSalesOrderRequest) GetProductsIdsOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ProductsIds, true
}

// SetProductsIds sets field value
func (o *NetSuiteExportSalesOrderRequest) SetProductsIds(v []string) {
	o.ProductsIds = v
}

// GetSubsidiaryId returns the SubsidiaryId field value
func (o *NetSuiteExportSalesOrderRequest) GetSubsidiaryId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SubsidiaryId
}

// GetSubsidiaryIdOk returns a tuple with the SubsidiaryId field value
// and a boolean to check if the value has been set.
func (o *NetSuiteExportSalesOrderRequest) GetSubsidiaryIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SubsidiaryId, true
}

// SetSubsidiaryId sets field value
func (o *NetSuiteExportSalesOrderRequest) SetSubsidiaryId(v string) {
	o.SubsidiaryId = v
}

func (o NetSuiteExportSalesOrderRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o NetSuiteExportSalesOrderRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["contractId"] = o.ContractId
	toSerialize["productsIds"] = o.ProductsIds
	toSerialize["subsidiaryId"] = o.SubsidiaryId
	return toSerialize, nil
}

func (o *NetSuiteExportSalesOrderRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"contractId",
		"productsIds",
		"subsidiaryId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varNetSuiteExportSalesOrderRequest := _NetSuiteExportSalesOrderRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varNetSuiteExportSalesOrderRequest)

	if err != nil {
		return err
	}

	*o = NetSuiteExportSalesOrderRequest(varNetSuiteExportSalesOrderRequest)

	return err
}

type NullableNetSuiteExportSalesOrderRequest struct {
	value *NetSuiteExportSalesOrderRequest
	isSet bool
}

func (v NullableNetSuiteExportSalesOrderRequest) Get() *NetSuiteExportSalesOrderRequest {
	return v.value
}

func (v *NullableNetSuiteExportSalesOrderRequest) Set(val *NetSuiteExportSalesOrderRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableNetSuiteExportSalesOrderRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableNetSuiteExportSalesOrderRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNetSuiteExportSalesOrderRequest(val *NetSuiteExportSalesOrderRequest) *NullableNetSuiteExportSalesOrderRequest {
	return &NullableNetSuiteExportSalesOrderRequest{value: val, isSet: true}
}

func (v NullableNetSuiteExportSalesOrderRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNetSuiteExportSalesOrderRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


