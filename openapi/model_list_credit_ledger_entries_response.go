/*
Vayu API

The Vayu API is a RESTful API that allows you to submit events for processing and storage & manage billing related entities.           The API is secured using the Bearer Authentication scheme with JWT tokens.           To obtain a JWT token, please contact Vayu at team@withvayu.com

API version: 1.0.0
Contact: dev@withvayu.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ListCreditLedgerEntriesResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListCreditLedgerEntriesResponse{}

// ListCreditLedgerEntriesResponse struct for ListCreditLedgerEntriesResponse
type ListCreditLedgerEntriesResponse struct {
	// The credit ledger entries for the customer.
	Entries []CreditLedgerEntry `json:"entries"`
}

type _ListCreditLedgerEntriesResponse ListCreditLedgerEntriesResponse

// NewListCreditLedgerEntriesResponse instantiates a new ListCreditLedgerEntriesResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListCreditLedgerEntriesResponse(entries []CreditLedgerEntry) *ListCreditLedgerEntriesResponse {
	this := ListCreditLedgerEntriesResponse{}
	this.Entries = entries
	return &this
}

// NewListCreditLedgerEntriesResponseWithDefaults instantiates a new ListCreditLedgerEntriesResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListCreditLedgerEntriesResponseWithDefaults() *ListCreditLedgerEntriesResponse {
	this := ListCreditLedgerEntriesResponse{}
	return &this
}

// GetEntries returns the Entries field value
func (o *ListCreditLedgerEntriesResponse) GetEntries() []CreditLedgerEntry {
	if o == nil {
		var ret []CreditLedgerEntry
		return ret
	}

	return o.Entries
}

// GetEntriesOk returns a tuple with the Entries field value
// and a boolean to check if the value has been set.
func (o *ListCreditLedgerEntriesResponse) GetEntriesOk() ([]CreditLedgerEntry, bool) {
	if o == nil {
		return nil, false
	}
	return o.Entries, true
}

// SetEntries sets field value
func (o *ListCreditLedgerEntriesResponse) SetEntries(v []CreditLedgerEntry) {
	o.Entries = v
}

func (o ListCreditLedgerEntriesResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListCreditLedgerEntriesResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["entries"] = o.Entries
	return toSerialize, nil
}

func (o *ListCreditLedgerEntriesResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"entries",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varListCreditLedgerEntriesResponse := _ListCreditLedgerEntriesResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varListCreditLedgerEntriesResponse)

	if err != nil {
		return err
	}

	*o = ListCreditLedgerEntriesResponse(varListCreditLedgerEntriesResponse)

	return err
}

type NullableListCreditLedgerEntriesResponse struct {
	value *ListCreditLedgerEntriesResponse
	isSet bool
}

func (v NullableListCreditLedgerEntriesResponse) Get() *ListCreditLedgerEntriesResponse {
	return v.value
}

func (v *NullableListCreditLedgerEntriesResponse) Set(val *ListCreditLedgerEntriesResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableListCreditLedgerEntriesResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableListCreditLedgerEntriesResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListCreditLedgerEntriesResponse(val *ListCreditLedgerEntriesResponse) *NullableListCreditLedgerEntriesResponse {
	return &NullableListCreditLedgerEntriesResponse{value: val, isSet: true}
}

func (v NullableListCreditLedgerEntriesResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListCreditLedgerEntriesResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


